...
const Painter = () => {
  ...
  const painters = workerRecords.filter(
    (record) => record.designation === "Painter"
  );

  const sortedPainters = [...painters].sort((a, b) => {
    ...
    if (sortOption === "fees") {
      return b.per_hour_fees - a.per_hour_fees;
    }
    return 0;
  });
  ...
  return (
    <div className="worker-main">
      <div className="worker-head-sec">
        <h1 className="worker-heading">Painter</h1>
      ...
      {sortedPainters && sortedPainters.length > 0 ? (
        <div className="worker-card-container">
          {sortedPainters.map((record) => (
